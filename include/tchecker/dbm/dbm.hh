...
namespace tchecker {
  ...
  namespace dbm {
    ...
    /*!
      \brief Checks subsumption or equivalence of two DBMs on ALU with global clock bounds.
      \param db1: DBM of first zone Z1
      \param db2: DBM of second zone Z2
      \param dim1: Dimension of first DBM
      \param dim2: Dimension of second DBM
      \param l: clock lower bounds
      \param u: clock upper bounds
      \param subsumption: bool indicating subsumption or equivalence.
      \return Given db1 and db2, check whether Z2<=Z1 or Z1~Z2, depending on the value of the
      subsumption. If subsumption is true, then we return true if Z2 is not simulated by Z1,
      otherwise we return false. If subsumption is false, then we return false if Z2~Z1, else
      we return true.
    */
