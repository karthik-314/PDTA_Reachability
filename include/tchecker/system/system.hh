...
namespace tchecker {
  ...
  protected:
    ...
    {
      // std::cout << "LOCATION\t" << typeid(loc).name() << "\n";
      try {
    ...
    void clone_locations_and_edges(tchecker::system_t<LOC, EDGE> & clone) const
    {
      ...
      for (EDGE const * edge : _edges) {
        ...
        new_edge->_stop = edge->_stop;
        new_edge->_symbol = edge->_symbol;
        new_edge->_push_or_pop = edge->_push_or_pop;
        new_edge->_constraint = edge->_constraint;
        new_edge->_fin = edge->_fin;
