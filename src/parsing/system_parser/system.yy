...
%type <tchecker::parsing::stack_op *>                            	sop
...
| TOK_EDGE ":" TOK_ID ":" TOK_ID ":" TOK_ID ":" TOK_ID attr_list sop "\n"
{
    ...
    if (src == nullptr)
      ...
      if (tgt == nullptr)
        ...
        if (event == nullptr)
          ...
          try {
          	tchecker::parsing::stack_op *sp = $11;
            auto * d = new tchecker::parsing::edge_declaration_t(*proc, *src, *tgt, *event, std::move($10), *sp);
...
sop:
"[" TOK_PUSH  ":" TOK_ID "]"  {
  if($4 == "")  error(@4,"empty stack symbol");
  else {
    std::stringstream value_loc;
    value_loc << @4;
    $$ = new tchecker::parsing::stack_op("push", $4, "push", value_loc.str());
  }
}
| "[" TOK_POP ":" TOK_ID TOK_LEQ TOK_INTEGER "]" {
  if($4 == "")  error(@4,"empty stack symbol");
  else {
    ...
    $$ = new tchecker::parsing::stack_op("pop", $4 + "<=" + $6, "pop", value_loc.str());
  }
}
| "[" TOK_POP ":" TOK_ID TOK_GEQ TOK_INTEGER "]" {
  if($4 == "")  error(@4,"empty stack symbol");
  else {
    std::stringstream value_loc, first, second;
    std::string first1 = ">=";
...
| "[" TOK_POP ":" TOK_ID TOK_LT TOK_INTEGER "]" {
  if($4 == "")  error(@4,"empty stack symbol");
  else {
    std::stringstream value_loc, first, second;
    std::string first1 = "<";
...
| "[" TOK_POP ":" TOK_ID TOK_GT TOK_INTEGER "]" {
  if($4 == "")  error(@4,"empty stack symbol");
  else {
    std::stringstream value_loc, first, second;
    std::string first1 = ">";
...
| "[" "]" {
  $$ = new tchecker::parsing::stack_op("nop", "", "nop", "");
}
;
