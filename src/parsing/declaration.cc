...
namespace tchecker {
  ...
  namespace parsing {
    ...
    stack_op::stack_op(std::string const &key, std::string const & value, std::string const & key_context, std::string const & value_context)
    : _key(key),
    _value(value),
    _key_context(key_context),
    _value_context(value_context)
    {
      // std::cout << "STACK OPERATION###################\n" << key << " " << value << "\n####################\n";
    }

    stack_op::stack_op(){}

    std::ostream & operator<< (std::ostream & os, tchecker::parsing::stack_op const & sop){
      os << "[" << sop.key() << ": " << sop.value() << "]";
      return os;
    }
    ...
    {
      tchecker::parsing::attributes_t attr(_attr);
      return new tchecker::parsing::edge_declaration_t(_process, _src, _tgt, _event, std::move(attr), _stack_op);
    }
