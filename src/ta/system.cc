...
namespace tchecker {
  ...
  namespace ta {
    ...
    void system_t::add_edge(std::string const & process_name, std::string const & src_name, std::string const & tgt_name,
                            std::string const & event_name, tchecker::expression_t * guard, tchecker::statement_t * statement, bool stop,
                            bool push_or_pop, std::string symbol, int constraint, int fin)
    {
      ...
      try {
        ...
        // Create the edge
        edge = new tchecker::ta::edge_t(pid, id, src, tgt, event_id, guard, statement, stop, push_or_pop, symbol, constraint, fin);
